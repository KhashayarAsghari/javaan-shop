...
import { DeleteSvg, FiltersSvg, SearchGreenSvg, ToomanSvg } from "../../PublicComponents/SVGs";
import {
  allBrands,
  allColors,
  allSellers,
...
export default function Filters() {
  ...
  const [isAvailableProductsChecked, setIsAvailableProductsChecked] =
    useState(false);
    ...
    <>
      <Box className="sticky top-3" sx={{ padding: "24px 24px 16px 16px" }}>
        <form className="w-full" onSubmit={(e) => e.preventDefault}>
          ...
          <Button
            className="w-full mt-2 flex justify-center gap-2 mx-auto"
            variant="text"
            color="error"
            endIcon={<DeleteSvg />}
          >
          ...
          <Box>
              ...
              <Collapse in={isBrandsOpen} timeout="auto" unmountOnExit>
                ...
                <List component="div" disablePadding>
                  {allBrands.map((item) => {
                    return (
                      <ListItemButton
                        onClick={handleToggleBrands(item.id)}
                        key={item.id}
                        dense
                        sx={{
                          display: "flex",
              ...
              <Collapse in={isSellersOpen} timeout="auto" unmountOnExit>
                <List component="div" disablePadding>
                  {allSellers.map((item) => {
                    return (
                      <ListItemButton
                        onClick={handleToggleSellers(item.id)}
                        key={item.id}
                        dense
                        sx={{
                          display: "flex",
              ...
              <Collapse in={isColorsOpen} timeout="auto" unmountOnExit>
                <List component="div" disablePadding>
                  {allColors.map((item) => {
                    return (
                      <ListItemButton
                        onClick={handleToggleColors(item.id)}
                        key={item.id}
                        dense
                        sx={{
                          display: "flex",
                      ...
                      >
                        ...
                        <Box
                          component="div"
                          className="w-5 h-5 rounded-sm border-2 border-black"
            ...
            <FormControlLabel
              ...
              control={
                <FastSendSwitch
                  checked={isFastSendChecked}
                  onChange={handleFastSendChange}
                  name="isFastSendChecked"
                />
              }
              label="ارسال فوری"
            />
            ...
            <FormControlLabel
              ...
              control={
                <FastSendSwitch
                  ...
                  onChange={() =>
                    setIsAvailableProductsChecked(!isAvailableProductsChecked)
                  }
                  name="isFastSendChecked"
                />
              }
